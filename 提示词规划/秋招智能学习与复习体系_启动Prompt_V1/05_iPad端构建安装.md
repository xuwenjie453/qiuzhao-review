...
# 2. 构建（Automatic 签名，Team GUS56DYZ6W；需本机有开发者证书）
...
Bundle ID：`local.qiuzhaoreview.ipad`。首启会出现"本地网络用途说明"页，点继续即触发 Bonjour 发现。
...
xcrun simctl terminate "iPad Air 11-inch (M4)" local.qiuzhaoreview.ipad 2>/dev/null
xcrun simctl launch  "iPad Air 11-inch (M4)" local.qiuzhaoreview.ipad   # 离线显示种子图
```
