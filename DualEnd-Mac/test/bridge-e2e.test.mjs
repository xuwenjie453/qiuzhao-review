...
test('1-2. connect + HELLO → WELCOME(新 session_epoch)', async () => {
  client.sendJson({
    v: 3, message_id: uid(), type: 'HELLO', session_epoch: null, sent_at: new Date().toISOString(),
    payload: { device_id: 'test-ipad-A', client_build: '3.0.0', supported_protocols: [3], last_server_seq: 0, cached_graph: null },
  });
  const w = await client.waitJson((m) => m.type === 'WELCOME');
  assert.equal(w.payload.selected_protocol, 3);
...
test('6. CLIENT_COMMAND move → COMMAND_ACK(canonical) + patch', async () => {
  ...
  client.sendJson({
    v: 3, message_id: uid(), type: 'CLIENT_COMMAND', session_epoch: null, sent_at: new Date().toISOString(),
  ...
  const canonical = await fetch(`http://127.0.0.1:${controlPort}/graph/snapshot`).then((r) => r.json());
  const moved = canonical.nodes.find((n) => n.node_id === center.node_id);
  assert.equal(moved.layout.x, 42_000);
  assert.equal(moved.layout.y, -17_000);
});
...
test('7. 重复 command → one effect', async () => {
  ...
  client.sendJson({ v: 3, message_id: uid(), type: 'CLIENT_COMMAND', session_epoch: null, sent_at: new Date().toISOString(), payload });
  client.sendJson({ v: 3, message_id: uid(), type: 'CLIENT_COMMAND', session_epoch: null, sent_at: new Date().toISOString(), payload }); // 重复
...
test('8. 重连 → 新 session_epoch 隔离', async () => {
  ...
  c2.sendJson({ v: 3, message_id: uid(), type: 'HELLO', session_epoch: null, sent_at: new Date().toISOString(), payload: { device_id: 'test-ipad-A', client_build: '3.0.0', supported_protocols: [3], last_server_seq: 0, cached_graph: { graph_id: 'g', revision: 0 } } });
...
test('9. revision mismatch → 请求 snapshot 恢复', async () => {
  ...
  // snapshot 请求恢复
  const snapP = client.waitJson((m) => m.type === 'GRAPH_SNAPSHOT');
  client.sendJson({ v: 3, message_id: uid(), type: 'SNAPSHOT_REQUEST', session_epoch: null, sent_at: new Date().toISOString(), payload: { graph_id: snap.graph_id } });
