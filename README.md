...
- 启动：`./DualEnd-Mac/bin/daemon-fixed.sh start`（**固定端口 57689**，以 macOS `launchd` 用户服务托管，iPad 端静态直连 `192.168.1.197:57689` 依赖它；勿用裸 `daemon start` 随机端口）；Agent 协作契约见 RuntimePrompt `21_双端问题图与iPad协作协议.md`；验收与状态见 `双端Release验收清单.md`、`双端更新实施状态.md`。
...
- 2026-09-16：**节点形状与 kind 解耦**（`shape: □/○/△`，任意节点含 CENTER 可改；协议 v4 / schema v6；旧图迁移保持原视觉）。Mac 75/75、iPad 56/56 测试绿。（同日曾实施独立笔记本体系，随后按用户决定移除，仅保留形状能力。）
